...
use DB;
class ApprovedProcurements extends BaseWidget
{
    ...
    protected function getTableQuery(): Builder
    {
        if(auth()->user()->user_role == 0) {
            return DepartmentProcurement::query()->where('status', 1)->latest();
        } else if (auth()->user()->user_role == 1) {
            return DepartmentProcurement::query()->where('status', 1)->latest();
        } else {
            return DepartmentProcurement::query()->where('status', 1)->where('ddo_code', auth()->user()->ddo_code)->latest();
        }
    }
