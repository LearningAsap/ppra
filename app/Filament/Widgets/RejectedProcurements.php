...
use DB;
class RejectedProcurements extends BaseWidget
{
    ...
    protected function getTableQuery(): Builder
    {
        if(auth()->user()->user_role == 0) {
            return DepartmentProcurement::query()->where('status', 0)->latest();
        } else if (auth()->user()->user_role == 1) {
            return DepartmentProcurement::query()->where('status', 0)->latest();
        } else {
            return DepartmentProcurement::query()->where('status', 0)->where('ddo_code', auth()->user()->ddo_code)->latest();
        }
    }
